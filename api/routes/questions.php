...
Flight::route("GET /user/questions", function () {
    ...
    $total = Flight::questionService()->get_questions(
        Flight::get("user")["id"],
        $offset,
        $limit,
        $search,
    ...
    Flight::json(Flight::questionService()->get_questions(
        Flight::get("user")["id"],
        $offset,
        $limit,
        $search,
...
Flight::route("GET /user/questions/hot", function () {
    $status = Flight::query('status', 'ACTIVE');
    Flight::json(Flight::questionService()
        ->get_weeks_hottest_questions($status, Flight::get("user")["d_id"]));
});
...
Flight::route("GET /user/questions-count", function () {
    Flight::json(Flight::questionService()
        ->get_question_count(Flight::get("user")["id"]));
});
...
Flight::route("GET /user/questions/@id", function ($id) {
    Flight::json(Flight::questionService()
        ->get_questions_by_question_id(Flight::get("user")["id"], $id));
});
...
Flight::route("GET /questions/@id", function ($id) {
    Flight::json(Flight::questionService()
        ->get_questions_for_course(100, 0, '+id', $id, 'ACTIVE'));
});
...
Flight::route("PUT /user/questions/@id", function ($id) {
    $data = Flight::request()->data->getData();
    Flight::json(Flight::questionService()->update_question(
        Flight::get("user"),
        $id,
        $data
    ));
...
Flight::route("PUT /admin/questions/@id", function ($id) {
    Flight::json(Flight::questionService()->update(
        $id,
