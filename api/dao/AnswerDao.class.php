...
class AnswerDao extends BaseDao
{
    ...
    public function get_answer_by_answer_id($user_id, $id)
    {
        return $this->query_unique(
            "SELECT answers.*, users.name FROM answers 
            JOIN users ON answers.user_id=users.id 
            WHERE answers.id = :id AND 
    ...
    public function get_answer_count($user_id)
    {
        return $this->query_unique(
            ...
            AND is_pinned = 1) AS pins FROM answers 
    ...
    public function get_answer_by_question_id($id, $order, $status)
    {
        ...
        $query .= " AND question_id = :question_id 
            ORDER BY answers.${order_column} ${order_direction}, 
            answers.posted_at ${order_direction}";
