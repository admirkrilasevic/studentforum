...
class QuestionDao extends BaseDao
{
    ...
    public function get_questions_for_departments(
        $limit,
        $offset,
        $order = "-id",
        $department_id,
    ...
    public function get_questions_for_course(
        $limit,
        $offset,
        $order = "-id",
        $course_id,
    ...
    {
        return $this->query_unique("SELECT * FROM questions WHERE id = :id 
			AND user_id = :user_id", ["id" => $id, "user_id" => $user_id]);
    }

    public function get_weeks_hottest_questions($status, $department_id)
    {
        return $this->query(
            "SELECT questions.*, users.name AS name FROM questions 
			JOIN users ON questions.user_id=users.id 
			WHERE questions.posted_at > NOW() - INTERVAL 7 DAY 
			AND questions.status = :status AND questions.department_id = :department_id",
    ...
    {
        return $this->query_unique("SELECT COUNT(*) AS count FROM questions 
